...
> {
	...
	protected abstract queryRemapFilter(
		filter?: Record<string, unknown>,
	...
	async query(
		query: DatabaseQuery,
		filterProps?: DatabaseDTOProperties[],
		...
		const res = await rateLimitedQuery(
			notionDatabaseQueryURL(
				...
				this.queryRemapFilterProperties(filterProps),
			),
			{
				method: 'POST',
	...
	async updatePage(
		id: string,
		patch: DatabasePatchDTO | UpdatePageBodyParameters,
	...
	async getPageBlocks(
		id: string,
		opts: ListBlockChildrenQueryParameters = { page_size: 100 },
	): Promise<Array<BlockObjectResponseWithChildren>> {
