...
const Hero = () => {
	const { loading, error, data } = useFetch("http://localhost:1337/api/section-heroes?populate=*");
	let heroData;
	if (data) {
		heroData = data.data[0].attributes;
		console.log(heroData.heroImg.data.attributes.url);
	}
	return (
				...
				<div className="shrink-0 rounded-full bg-gradient-to-r from-cyan-500  to-fuchsia-500">
					{data && (
						<Image
							...
							width={350}
							height={350}
							src={`http://localhost:1337${heroData.heroImg.data.attributes.url}`}
							alt="Hero img"
						></Image>
					)}
				</div>
